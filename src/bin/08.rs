...
pub fn part_two(input: &str) -> Option<usize> {
    ...
    let ends: Vec<usize> = starts.iter()
        .map(|s|
            count_steps(&network, &steps, s, |e| e.ends_with('Z'))
        ).collect();
...
fn parse(input: &str) -> Input {
    ...
    let steps = lines.next().expect("first line contains L/R steps").to_string();
    lines.next();
    let network = HashMap::from_iter(lines
        .map(|line| {
